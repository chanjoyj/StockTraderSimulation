...
public class SetToday {
	...
	public static boolean checkValidDate(String date) {
		...
		if (dateArr[0].length() == 2 && dateArr[1].length() == 2 && dateArr[2].length() == 4
			...
			// is a date now check if is leap year date
			if ((Integer.parseInt(dateArr[2]) % 400 == 0
					|| (Integer.parseInt(dateArr[2]) % 4 == 0 && Integer.parseInt(dateArr[2]) % 100 != 0)) && // is a
																												// leap
																												// year
					Integer.parseInt(dateArr[1]) == 2 && Integer.parseInt(dateArr[0]) > 29) {
				return false;
			} else if (Integer.parseInt(dateArr[1]) == 2 && Integer.parseInt(dateArr[0]) > 28) {// is feb and not leap
																								// year
				return false;
			} else
				today = changeDate(date);
